...
function calculateStreak(logs: { date: Date; kept: boolean }[]): number {
  const kept = logs
    .filter((l) => l.kept)
  ...
  if (!isSameDay(kept[0], check) && !isSameDay(kept[0], subDays(check, 1))) return 0;
...
server.tool(
  ...
  async ({ mealType, description, calories, protein, carbs, fat, notes }) => {
    ...
    return { content: [{ type: "text" as const, text: `Logged meal: ${meal.id}` }] };
  }
);
...
server.tool(
  ...
  async ({ name, type, durationMin, notes }) => {
    ...
    return { content: [{ type: "text" as const, text: `Logged workout: ${workout.id}` }] };
  }
);
...
server.tool(
  ...
  async ({ category, title, durationMin, resource, notes }) => {
    ...
    return { content: [{ type: "text" as const, text: `Logged session: ${session.id}` }] };
  }
);
...
server.tool(
  ...
  async ({ habitId, kept, notes }) => {
    ...
    if (!habit) return { content: [{ type: "text" as const, text: "Habit not found" }] };
...
server.tool(
  ...
  async ({ period }) => {
    ...
    const avgCalories =
      meals.filter((m) => m.calories).length > 0
        ? Math.round(
    ...
    const habitStats = habitLogs.reduce<
      Record<string, { name: string; kept: number; missed: number }>
    >((acc, log) => {
      if (!acc[log.habitId]) {
        acc[log.habitId] = { name: log.habit.name, kept: 0, missed: 0 };
      }
      if (log.kept) acc[log.habitId].kept++;
      else acc[log.habitId].missed++;
      return acc;
    }, {});
